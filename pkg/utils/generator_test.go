...
func TestGenerateSessionToken(t *testing.T) {
	...
	token := gen.GenerateSessionToken()
	if len(token) == 0 {
	...
	if !hexRegex.MatchString(token) {
		t.Errorf("Session token should be hexadecimal: %s", token)
	}
}
...
func TestGenerateRefreshToken(t *testing.T) {
	...
	token := gen.GenerateRefreshToken()
	if len(token) == 0 {
	...
	if !base64Regex.MatchString(token) {
		t.Errorf("Refresh token should be base64 encoded: %s", token)
	}
}
